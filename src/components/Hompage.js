...
function Homepage() {
  ...
  const [gType, setGType] = useState('type'); // Gebäudetyp
  ...
  const successAlert = () => {
    ...
    if(jahr !== '' && jahr >=1800 && gType !== 'type' && wGesamt !== ''){
      Swal.fire({
        title: 'Erfolg!',
    ...
    else if(jahr === '' || gType === 'type' || wGesamt === ''){
      Swal.fire({
        title: 'Fehler!',
  ...
  async function getInfo(construction_year, building_type){
    ...
    const url = "https://energyark.advisore.eu/api/modernization-reference/";
    const headers = {
      'Content-Type': 'application/json',
      'Authorization': `Token ${token}`
    }
  ...
  const formSubmit = (e) => {
    ...
    if(construction_year >= 1000 || building_type !== "type"){
    ...
    if(data!==null){
      ...
      const {gross_energy_roomheating_values, gross_energy_waterheating_values} = statistical_consumption_data;
      const {cellarroof, hull, roof, windows, ventilation} = references;
      setFassadeDesc(capitalize(hull[fassade]?.description));
      setDachDesc(capitalize(roof[dach]?.description));
      setFensterDesc(capitalize(windows[fenster]?.description));
      setKellerDesc(capitalize(cellarroof[keller]?.description));
      setLüftungDesc(capitalize(ventilation[lüftung]?.description));
      setDachLimit(roof.length - 1);
      setFassadeLimit(hull.length - 1);
      setFensterLimit(windows.length - 1);
      setKellerLimit(cellarroof.length - 1);
      ...
      <div className='w-full h-50 px-3 pb-3 space-y-2 rounded-xl  '>
          <div className='flex items-center justify-end space-x-3 pr-7 pt-1'>
              <h1 className='text-green-600 text-xl'>powered by</h1>
                <a href='https://bauing.rptu.de/ags/ioe/fg-immobilienoekonomie' target='_blank'>
                  <img src={require("../img/logo_ioe.png")} alt='logo' className=' h-32 w-44'/>
                </a>
              <div className='flex justify-center items-end space-x-2 cursor-pointer'>
                <p className=' text-5xl'>|</p>
                <a href='https://advisore.eu/?page_id=22' target='_blank'>
                  <img src={require("../img/EnergyARK_Full.png")} alt='logo' className=' h-10 w-52'/>
                </a>
                <p className=' text-5xl'>|</p>
                <a href='https://advisore.eu/' target='_blank'>
                  <img src={require("../img/Logo_no_background.png")} alt='logo' className=' h-10 w-52'/>
                </a>
              </div>
          </div>
          <div className=''>
          ...
          <form className='w-full' id='gebäude hinzufüngen' onSubmit={formSubmit}>
              <div className='space-y-2'>
                <div className='w-full flex-col space-x-12 space-y-14'>
                  ...
                  <div className='relative'>
                    <input type='number' min={0} onChange={(e)=>setJahr(e.target.value)} onKeyPress={preventMinus} id='Baujahr' placeholder='Wohnungen gesamt' className='h-10 px-2 bg-neutral-100 placeholder-transparent focus:outline-none border-b-2 focus:border-green-600 transition-colors peer' required></input>
                  ...
                  <div className='relative w-72'>
                    <select defaultValue="181.39" onChange={(e)=>setEmissionfactor(e.target.value)} id="underline" className="block cursor-pointer py-2.5 px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-gray-200 peer" required> 
                        ...
                        <option value="235.58">Emissionsfaktor Flüssiggas: 235,58 g/kWh</option>
                        <option value="240">Emissionsfaktor Erdgas: 240 g/kWh</option>
                        <option value="290">Emissionsfaktor Flüssiggas: 290 g/kWh</option>
      ...
      <div className='pt-5 flex w-full justify-center'>           
        <div className='pt-2 w-3/5 shadow-2xl bg-slate-200 rounded-lg'>
          ...
          <div className='mr-2 ml-2 p-10 mb-4 justify-center items-center'>
            ...
            />
            <Accordion 
              question="Wie interpretiere ich die Zahlen richtig?" 
              answer="Die Zahlen entsprechen realistischen und real zu beobachtenden Verbrauchswerten.
              Deshalb wird bereits ein sparsames Verbrauchsverhalten bei energetisch schlechten (Preboundeffekt) und ein erhöhter Verbrauch bei energetisch hochwertigen Gebäuden (Reboundeffekt) mit einberechnet.
              Der Energieverbrauch umfasst Wärme für Raumwärme und Warmwasser."
              count={3}
            />
            <Accordion
              question="Welchen Emissionsfaktor soll ich auswählen?"
              answer={`Verwirrenderweise gibt es eine Vielzahl von Emissionsfaktoren für die jeweiligen Energieträger. Die Unterschiede ergeben sich aus unterschiedlichen Definitionen für die Energiemengen (z.B. Brennwert vs. Heizwert) und der Berücksichtigung weiterer Prozessschritte zur Bereitstellung des Energieträgers (z.B. Emissionen aus Raffinerien oder dem Transport). 
                      Deshalb ist die Auswahl des richtigen Emissionsfaktors für Ihre Anwendung entscheidend für die Aussagekraft des Ergebnisses.
                        - Erdgas_Hs (181,39 g/kWh): Brennwertbezogener Emissionsfaktor (nach EBeV 2030) zur Berechnung der CO2-Kosten (CO2KostAufG)
                        - Heizöl (286,92 g/kWh): Emissionsfaktor (nach EBeV 2030) zur Berechnung der CO2-Kosten (CO2KostAufG)
                        - Flüssiggas (235,58 g/kWh): Emissionsfaktor (nach EBeV 2030) zur Berechnung der CO2-Kosten (CO2KostAufG)
                        - Erdgas_Hi (200,88 g/kWh): Heizwertbezogener Emissionsfaktor (nach EBeV 2030) zur Berechnung der CO2-Kosten (CO2KostAufG)
                        - Erdgas (240 g/kWh): Emissionsfaktor nach GEG
                        - Flüssiggas (290 g/kWh): Emissionsfaktor nach GEG
                        - Heizöl (310 g/kWh): Emissionsfaktor nach GEG`}              
              count={4}
            />
          </div>
        </div>
